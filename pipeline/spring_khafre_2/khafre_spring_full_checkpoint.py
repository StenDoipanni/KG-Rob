...
def get_amr_from_spring_parser(text, spring_model=None, output_dir="./out/amr/"):
    ...
    try:
        ...
        # Import SPRING-specific modules
        from spring_amr.penman import encode
        ...
        # Decode the outputs
        decoded_amr = model.decode_amr(outputs.cpu().numpy(), restore_name_ops=True)
        amr_output = encode(decoded_amr[0])
...
def main():
    ...
    if args.use_spring and spring_model is not None:
        ...
        if args.use_spring:
            logger.warning("SPRING model loading failed, using simple AMR generator as fallback")
        else:
            logger.info("Using simple AMR generator as configured")
